...
| 경로 | 조건 | 캡처 |
|---|---|---|
| 정상/무시 | AdMob `optional` 계정(원래 빈 계정)이 `미사용`으로 정상 처리 | ![AdMob optional 정상 처리](./screenshots/p2-branch-admob-optional.png) |
| 에러/알림 | AdMob 실계정 실패가 `error`로 기록·표시 | ![AdMob 실계정 에러](./screenshots/p2-branch-admob-error.png) |
